...
// Servo calibration offset: Adjust this value if sensor doesn't face straight
// Positive value = rotate left, Negative value = rotate right
// If sensor points 15° to the right, set to +15. If 15° to the left, set to -15
#define SERVO_Z_OFFSET 15  // Corrects 15° right offset - adjust as needed
};
/*Buzzer - Simple passive buzzer driver*/
class DeviceDriverSet_Buzzer
{
public:
  void DeviceDriverSet_Buzzer_Init(void);
  void DeviceDriverSet_Buzzer_Beep(uint16_t freq, uint16_t duration);
  void DeviceDriverSet_Buzzer_Off(void);
private:
#define PIN_Buzzer A3  // Buzzer pin (connect buzzer + to A3, - to GND)
};

/*LCD Display - Grove RGB LCD (memory optimized)*/
#include "rgb_lcd.h"
class DeviceDriverSet_LCD
{
public:
  void DeviceDriverSet_LCD_Init(void);
  void DeviceDriverSet_LCD_Update(void);  // Call in main loop for scrolling
  void DeviceDriverSet_LCD_SetMode(uint8_t mode);  // Update display for mode change
  void DeviceDriverSet_LCD_ObstacleAlert(bool detected);  // Show obstacle alert
private:
  void LCD_DisplayModeText(uint8_t mode);
  uint8_t scrollPos;
  uint8_t currentMode;
  unsigned long lastScrollTime;
  unsigned long lastColorTime;
  uint8_t colorPhase;
  bool obstacleAlert;
};
